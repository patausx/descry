...
- `song.dly_time` хранится как 0..255 квантов по 32 аудиофрейма; движок клампит минимум до 32 фреймов. При фиксированных 32 кГц UI показывает это как 1..255 ms. Это не tempo-sync.
...
**SONG-СТРОКА ИМЕЕТ ОДНУ ОБЩУЮ ГРАНИЦУ ДЛЯ ВСЕХ ТРЕКОВ.**
Её длительность — самый длинный чейн в строке, посчитанный в шагах с учётом `Phrase::length` (минимум 16 шагов). Короткий чейн циклически повторяется внутри строки; `EMPTY` молчит до общей границы. Поэтому треки всегда переходят на следующую Song-строку одновременно, а отдельный rest-чейн для синхронизации больше не нужен.
